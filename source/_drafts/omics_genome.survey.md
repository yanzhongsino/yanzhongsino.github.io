...
3. 修正基因组大小
在不考虑测序错误、序列重复性和杂合序列的条件下，K-mer的深度分布遵循泊松分布。但实际情况是三者都存在，所以需要进行修正。
- 测序错误：一般认为低频K-mer(K=1、2)是测序错误引起的，去除低频K-mer并计算错误率以修正基因组大小的估计。
- 杂合序列：基因组中存在的杂合序列会对应两种类型的K-mer，并使得这些K-mer频数低于K-mer期望深度D。假设两种K-mer数量大致相等，在K-mer频数分布图中杂合序列对应的K-mer会表现为频数约为主峰一半的一个峰。根据主峰左侧的杂合峰可以估计基因组的杂合度。
- 重复序列：基因组中存在的重复序列会使对应的K-mer频数高于K-mer期望深度D。在K-mer频数分布图中重复序列对应的K-mer会表现为频数大于主峰的一个峰。根据主峰右侧的重复峰可以估计基因组的重复序列比例。
...
- 将低频K-mer(比如K-mer深度等于1)的情况认为是错误，计算错误率，并用于修正基因组大小。
