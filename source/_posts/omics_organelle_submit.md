...
2. Contact：填写上传人的信息
3. Reference：填写参考文献信息
4. Sequencing Technology：填写测序方法信息，包括reads类别，是否是组装的数据，组装软件和版本，组装名称和深度。
5. Nucleotide：填写序列的信息。
- 分子类型（Molecule Type）：细胞器基因组选的genomic DNA
- Topology：线型分子（Linear）还是环形分子（Circular）。
- 是否是完整的细胞器基因组
...
8. Source Modifiers：资源信息，是叶绿体/线粒体，来自植物什么器官。
