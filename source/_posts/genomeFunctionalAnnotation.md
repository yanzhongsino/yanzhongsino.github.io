...
# 基因功能注释 gene functional annotation
[ref](https://www.jianshu.com/p/67dbafa86334)
...
### 下载数据库手动注释--以Uniprot为例
参考[徐洲更教程](http://xuzhougeng.top/archives/Function-anotation-with-swiss-prot-database)
1. 下载数据库
...
3. 用blastp比对蛋白序列protein.fa和数据库uniprot_plants
...
4. 比对结果中筛选每个query的最佳subject
jcvi可用conda安装
`python -m jcvi.formats.blast best -n 1 blastp.outfmt6`
生成blastp.outfmt6.best

5. 使用add_annotation_from_dat.py根据blastp输出从dat提取GO/KEGG/同源基因
...
##### eggNOG-mapper注释原理
[ref](https://www.jianshu.com/p/e646c0fa6443)
注释分为三步：
1. 序列比对
